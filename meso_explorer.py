...
# ── Paths ──────────────────────────────────────────────────────────────────
BASE_DIR   = r"Y:\long\publication_datasets\mesothelioma\SPATIAL133261"
TX_FILE    = rf"{BASE_DIR}\GSM9654052_SPATIAL133261_transcripts.parquet.gz"
HIST_ZIP   = rf"{BASE_DIR}\histoseg_outputs (14).zip"
...
print("Loading transcripts (30-60 s) ...")
...
# Map each transcript to its cell's structure and boundary distance
...
# Build per-gene lookup: gene -> DataFrame[x_location, y_location, structure, dist, cell_id]
...
# Per-structure cell count (for normalisation)
STRUCT_NCELLS = part_df.groupby('isoline_structure_name', observed=True).size().to_dict()

# Per-structure / per-distance-bin cell sets (precompute for speed)
# This is used in distance_curve to count total cells per bin
def get_cells_in_bin(sname, lo, hi):
    """Return number of cells in structure sname with boundary dist in [lo, hi)."""
    mask = (
    ...
    return part_df.index[mask]  # cell_ids
...
# ── Per-structure summary for bar chart ───────────────────────────────────
def per_structure_bar(gene_df, metric):
    ...
    for sname in STRUCTS:
        ...
        if metric == 'pct':
            ...
            vals.append(100.0 * n_expr / max(n_cells, 1))
...
# ── Per-structure inward density curve ────────────────────────────────────
def distance_curve(gene_df, sname, metric='pct', max_dist=200, n_bins=N_BINS):
    """
    gene_df: GENE_TX[gene] (all structures)
    Returns (midpoints, values) or (None, None).
    """
    sub_s = gene_df[
    ...
    for i in range(n_bins):
        ...
        # Cells in this bin (from partition)
        ...
        if metric == 'pct':
            # Expressing cells = unique cell_ids in tx_bin that are also in bin_cell_ids
...
# ── App layout ─────────────────────────────────────────────────────────────
app = dash.Dash(__name__, title='Meso Xenium Explorer')

app.layout = html.Div([

    html.Div([
        html.H2('Mesothelioma Xenium — Structure Explorer',
            ...
            dcc.Graph(id='spatial-plot',
                      ...
                      config={'scrollZoom': True,
                              'toImageButtonOptions': {
                                  'format': 'png', 'filename': 'meso_spatial',
        ...
        html.Div([
            dcc.Graph(id='stats-plot',
                      ...
                      config={'toImageButtonOptions': {
                          'format': 'png', 'filename': 'meso_stats', 'scale': 2}}),
        ], style={'width': '43%'}),
